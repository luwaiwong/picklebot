...
// ── Book-by-code request: a numeric activity code (== CourseId on the site) plus the Markham
// email + password to log in with for THIS run. Credentials are never persisted. ──
export const BookRequestSchema = z.object({
  code: z.string().trim().regex(/^\d+$/, 'code must be numeric'),
...
export type LogEvent =
  ...
  | { type: 'login'; state: 'logging-in' | 'logged-in' | 'login-failed' | 'error'; detail?: string; at: string };
